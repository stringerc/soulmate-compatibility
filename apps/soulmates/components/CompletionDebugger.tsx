...
interface CompletionDebuggerProps {
  responses: number[];
  answeredScenarios?: Set<number> | number[];
  onForceComplete: () => void;
}

export default function CompletionDebugger({ responses, answeredScenarios, onForceComplete }: CompletionDebuggerProps) {
  const [isExpanded, setIsExpanded] = useState(true); // Start expanded
  
  // Memoize analysis to prevent expensive recalculation on every render
  // Use answeredScenarios if provided for accurate tracking
  const analysis = useMemo(() => analyzeCompletion(responses, answeredScenarios), [responses, answeredScenarios]);
  ...
  return (
    <div className="fixed bottom-24 right-4 bg-white dark:bg-gray-800 border-2 border-red-500 rounded-lg shadow-2xl p-4 max-w-md z-[10000]">
